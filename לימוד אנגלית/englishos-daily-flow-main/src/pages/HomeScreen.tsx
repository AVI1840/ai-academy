...
import { BookOpen, Target, ChevronRight, Trophy, Zap, AlertTriangle, Brain } from 'lucide-react';
...
const HomeScreen = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Milestone overlay */}
      {showMilestone && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center" onClick={() => setShowMilestone(null)}>
          <div className="bg-card rounded-3xl p-10 text-center mx-6">
            ...
            <button onClick={() => setShowMilestone(null)} className="bg-primary text-primary-foreground font-semibold px-8 py-3 rounded-2xl">
              ממשיך! 🚀
            </button>
          </div>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-5 md:px-8 md:py-10 md:grid md:grid-cols-[1fr_380px] md:gap-8 flex flex-col pt-safe-top pb-8 pt-10">

        {/* ── LEFT / TOP: stats panel ── */}
        <div className="flex flex-col gap-4">
          ...
          {/* Streak */}
          <div className={`rounded-2xl p-5 ${streakAtRisk ? 'bg-orange-500/15 border border-orange-500/40' : 'bg-card'}`}>
          ...
          <div className="bg-card rounded-2xl p-4">
            ...
            <div className="flex gap-2 justify-between">
              {badges.map(b => (
                <div
                  key={b.count}
                  className={`flex flex-col items-center gap-1 flex-1 py-2 rounded-xl transition-all ${b.unlocked ? 'bg-primary/10' : 'opacity-30'}`}
                >
                  <span className="text-xl">{b.emoji}</span>
                  <span className={`text-xs font-medium ${b.unlocked ? 'text-primary' : 'text-muted-foreground'}`}>{b.count}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-center gap-8 mt-2 md:hidden">
            <button className="text-primary text-xs font-medium flex flex-col items-center gap-1">
              <BookOpen className="w-5 h-5" />
              בית
            </button>
            <button onClick={() => navigate('/dashboard')} className="text-muted-foreground text-xs font-medium flex flex-col items-center gap-1">
        ...
        {/* ── RIGHT / BOTTOM: session + actions ── */}
        <div className="flex flex-col gap-4 mt-4 md:mt-0">

          {/* Today's session card */}
          <div className="bg-card rounded-2xl p-6 flex-1">
            ...
            {sessionWords.length > 0 ? (
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-2xl font-bold text-foreground mb-1">
                    ...
                    {reviewCount > 0 ? `${reviewCount} חזרה` : ''}
                  </p>
              ...
              <div className="text-center py-4">
                ...
                <p className="text-muted-foreground font-medium">סיימת להיום!</p>
          ...
          <button
            ...
            className="w-full bg-primary text-primary-foreground font-semibold text-lg py-5 rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-40 animate-pulse-glow"
          >
            {sessionWords.length > 0
              ? streakAtRisk ? '🔥 שמור על הרצף — התחל!' : 'התחל Flashcards'
              : 'תרגל ביטויים'}
          ...
          <button
            ...
            className="w-full bg-card border border-primary/30 text-primary font-semibold text-lg py-5 rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-40"
          >
