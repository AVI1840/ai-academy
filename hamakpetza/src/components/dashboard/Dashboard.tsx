        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-8 pt-20 pb-16">
          <div className="text-center animate-slide-up">
            ...
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-card text-accent text-xs font-heading font-semibold mb-8">
              ...
              אקדמיית AI למובילי המגזר הציבורי
            </div>

            {/* Title */}
            <h1 className="font-heading font-black text-text leading-[0.9] tracking-tight mb-5" style={{ fontSize: 'clamp(3.5rem, 10vw, 7rem)' }}>
              המקפצה
            </h1>
            <p className="text-xl md:text-2xl text-muted font-heading font-medium mb-3 max-w-2xl mx-auto leading-snug">
              מערכת ההפעלה של מנהיגות AI בממשלת ישראל
            </p>
            <p className="text-sm text-muted/60 max-w-lg mx-auto mb-10 leading-relaxed">
              {totalCourses} יחידות מעשיות — מאוריינות בסיסית ועד RAG, ענן ממשלתי וניהול ידע.
              חינם לכל עובדי המדינה.
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-14">
              ...
              >
                ...
                <svg className="w-5 h-5 rotate-180 transition-transform group-hover:-translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
              </Link>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-8 py-12">
          ...
          <section className="mb-10 glass-card rounded-2xl p-6 sm:p-8 animate-fade-in" aria-label="ההתקדמות שלי">
            <div className="flex flex-wrap items-center gap-6">
              ...
              <div className="flex-1 min-w-[160px]">
                <p className="font-heading font-bold text-text text-2xl mb-1">{completedModules.length} מתוך {totalCourses} יחידות</p>
                <p className="text-sm text-muted">{overallPct === 100 ? '🎉 השלמת את כל הקורס!' : `${overallPct}% הושלם — ממשיכים!`}</p>
                <div className="mt-3 h-2 rounded-full bg-border overflow-hidden">
                  <div className="h-full rounded-full bg-accent transition-all duration-700" style={{ width: `${overallPct}%` }} />
                </div>
              </div>
              <SocialShare title="המקפצה — אקדמיית AI ממשלתית" url="https://avi1840.github.io/ai-academy/" description={`${overallPct}% — השלמתי ${completedModules.length} יחידות מתוך ${totalCourses} בקורס המקפצה`} variant="compact" />
            </div>
          </section>
        )}
        ...
        <section className="mb-14" aria-label="תחומי למידה">
          <div className="flex items-center justify-between mb-6">
            ...
            {selectedPath && (
              <button onClick={() => setSelectedPath(null)} className="text-xs text-accent hover:underline min-h-[44px] min-w-[44px] flex items-center justify-center">הצג הכל</button>
            )}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {DOMAINS.map(domain => {
              ...
              return <DomainCard key={domain.id} domain={domain} courses={selectedPath ? filteredCourses : courses} completedCount={completedCount} />;
            })}
          </div>
        </section>
            ...
            {COURSE_CATALOG.map((course, i) => {
              ...
              return (
                <Link key={course.slug} href={`/course/${course.slug}/`}
                  ...
                  tabIndex={isHighlighted ? 0 : -1} aria-disabled={!isHighlighted}
                ...
                >
                  ...
                  {domainInfo && <span className="text-xl hidden sm:block flex-shrink-0" aria-hidden="true">{domainInfo.icon}</span>}
                  <div className="flex-1 min-w-0">
                    <p className="font-heading font-semibold text-text text-sm sm:text-base leading-snug">{course.title}</p>
                    <p className="text-xs text-muted truncate mt-0.5 hidden sm:block">{course.description}</p>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className="text-xs text-muted hidden md:block px-2 py-1 rounded-lg bg-surface">{course.duration}</span>
                    <svg className="w-4 h-4 text-muted/30 group-hover:text-accent transition-all group-hover:-translate-x-1 rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                  </div>
                </Link>
              );
            })}
